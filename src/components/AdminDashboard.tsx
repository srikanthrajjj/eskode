...
import { FiUsers, FiBarChart2, FiSettings, FiFileText, FiBell, FiSearch, FiHome, FiLogOut, FiFolder, FiClock, FiCheckCircle, FiMessageSquare, FiZap } from 'react-icons/fi';
...
const AdminDashboard = ({
  ...
  const [aiAutoAssistEnabled, setAiAutoAssistEnabled] = useState(true); // State for AI auto-assist toggle
      ...
      >
        ...
        <div className="p-2">
          <div className="flex flex-col space-y-2">
            ...
            <button
              className={`flex items-center p-3 rounded-16px ${
                activeTab === 'ai-assistant' ? 'bg-primary-50 text-primary-600' : 'text-gray-600 hover:bg-gray-100'
              } ${isSidebarHovered ? 'justify-start' : 'justify-center'}`}
              onClick={() => setActiveTab('ai-assistant')}
            >
              <FiZap className="text-lg" />
              {isSidebarHovered && <span className="ml-3">AI Assistant</span>}
            </button>
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="bg-white p-4 shadow-sm flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={() => setActiveTab('ai-assistant')}
              className="flex items-center space-x-1 px-3 py-2 bg-blue-50 text-blue-600 rounded-full hover:bg-blue-100 transition-colors"
              title="AI Assistant"
            >
            ...
            {activeTab === 'ai-assistant' && (
              <div className="p-4 h-full">
                <div className="bg-white rounded-lg shadow-md h-full overflow-hidden">
                  <AIAssistant position="content-area" />
                </div>
              </div>
            )}
          </div>
