...
import { FiPaperclip, FiSend, FiArrowLeft, FiPhone, FiVideo, FiImage, FiMoreVertical, FiMic, FiPlay, FiPause } from 'react-icons/fi';
  ...
  useEffect(() => {
    const messageCallback = websocketService.onMessage((message) => {
      ...
      if (message.type === 'VICTIM_MESSAGE') {
        ...
        const victimMessage: ChatMessage = {
        ...
        setLocalMessages(prev => [...prev, victimMessage]);
      ...
      if (message.type === 'POLICE_TO_VICTIM_MESSAGE') {
        // Add message to local state
        const policeMessage: ChatMessage = {
          id: message.payload.id,
          sender: 'officer',
          senderName: message.payload.senderName,
          message: message.payload.message,
        ...
        setLocalMessages(prev => [...prev, policeMessage]);
      }
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Send message to victim if the handler is provided
        if (onSendMessageToVictim) {
          onSendMessageToVictim('Voice message');
        }
                      ...
                      >
                        {msg.type === 'voice' ? (
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {isPlaying === msg.id ? <FiPause /> : <FiPlay />}
                            </button>
                            <div className="flex-1">
                              <div className="w-full bg-gray-200 rounded-full h-1">
                                <div 
                                  className={`h-1 rounded-full ${
                                    msg.sender === 'officer' ? 'bg-[#128C7E]' : 'bg-gray-400'
                                  }`}
                                  style={{ width: '100%' }}
                                />
                              </div>
          ...
          <div className="flex items-center justify-between bg-white rounded-full px-4 py-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-[#128C7E] text-white px-4 py-1 rounded-full"
              >
                Send
              </button>
            </div>
