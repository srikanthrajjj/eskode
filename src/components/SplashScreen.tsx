...
const SplashScreen = ({ onSelectPoliceApp, onSelectAdminDashboard, onSelectVictimApp, onSelectAIAssistant }: SplashScreenProps) => {
  const [hoveredButton, setHoveredButton] = useState<'police' | 'admin' | 'victim' | 'ai-assistant' | null>(null);
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-900 via-primary-800 to-primary-700 flex flex-col items-center justify-center p-4 overflow-hidden relative">
      ...
      <div className="flex flex-col md:flex-row gap-6 w-full max-w-5xl justify-center px-4">
        ...
        {onSelectAIAssistant && (
          <button
            className={`w-full md:w-1/3 bg-white rounded-2xl p-5 shadow-lg transform transition-all duration-500 relative overflow-hidden ${
              hoveredButton === 'ai-assistant' ? 'scale-105 shadow-xl' : ''
            } ${animateIn ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}
            style={{ transitionDelay: '0.3s' }}
          ...
          >
            {/* Glow effect on hover */}
            <div
              className={`absolute inset-0 bg-gradient-to-r from-blue-300 to-purple-400 opacity-0 transition-opacity duration-500 ${hoveredButton === 'ai-assistant' ? 'opacity-10' : ''}`}
            ></div>

            <div className="bg-blue-100 text-blue-700 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 transform transition-all duration-500 relative">
              <div className={`absolute inset-0 rounded-full bg-blue-200 transform scale-0 transition-transform duration-500 ${hoveredButton === 'ai-assistant' ? 'scale-110' : ''}`}></div>
              <svg xmlns="http://www.w3.org/2000/svg" className={`w-8 h-8 transition-all duration-500 relative z-10 ${hoveredButton === 'ai-assistant' ? 'scale-125' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                <path d="M13 8l2 2"></path>
                <path d="M9 8h.01"></path>
              </svg>
            </div>
            <h2 className="text-lg font-bold text-gray-900 text-center mb-2">AI Assistant</h2>
            <p className="text-sm text-gray-600 text-center mb-4">
              Get instant help and answers to your questions with our AI-powered assistant.
            </p>
            <div className={`mt-4 bg-blue-600 text-white rounded-full py-2 px-6 text-center font-medium transition-all duration-500 ${
              hoveredButton === 'ai-assistant' ? 'bg-blue-700 shadow-lg' : ''
            }`}>
              Open Assistant
            </div>
          </button>
