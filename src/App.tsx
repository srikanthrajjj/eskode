...
type AppView = 'splash' | 'police' | 'admin' | 'victim' | 'ai-assistant';
...
function App() {
  ...
  const handleWebSocketMessage = (message: WebSocketMessage) => {
    ...
    switch (message.type) {
      ...
      case 'POLICE_TO_VICTIM_MESSAGE':
        // Handle both victim and police messages in the same way
        console.log(`Processing ${message.type} with payload:`, message.payload);

        // For police users, always handle messages
        if (currentView === 'police') {
          console.log('Police app: Processing message');
          handleIncomingVictimMessage(message.payload);
        }
        // For other users, check if this message is intended for current user
          ...
          console.log(`Ignoring ${message.type} - not addressed to current user`);
        }
        break;
  ...
  const handleIncomingAdminMessage = (payload: AdminChatMessage & { recipientId: string, taskRequest?: boolean }) => {
    console.log('Handling admin message:', payload);

    // Skip task request messages - they're handled separately
    if (payload.taskRequest) {
      console.log('Skipping task request message');
      return;
    }

    const { recipientId } = payload;

    // Only process messages intended for the current user or if we're the admin
    const isForCurrentUser =
      (currentView === 'police' && recipientId === 'off1') ||
      (currentView === 'admin');

    if (!isForCurrentUser) {
      console.log('Message not intended for current user, skipping');
      return;
    }
    ...
    setAdminMessages(prev => {
      ...
      const isDuplicate = existingMessages.some(msg => msg.id === payload.id);
      if (isDuplicate) {
    ...
    // Update unread count if we're in police view and chat is not open
  ...
  // Handle incoming messages (both from victim and police)
  const handleIncomingVictimMessage = (payload: any) => {
    console.log('Handling message in police app:', payload);

    // Get the caseId from the payload or use default
    const caseId = payload.caseId || 'case1';

    // Create a formatted message based on sender
    const formattedMessage = {
      id: payload.id || Date.now().toString(),
      sender: payload.sender || (payload.sender === 'officer' ? 'officer' : 'victim'),
      senderName: payload.sender === 'officer'
        ? (payload.senderName || 'S. Morgan')
        : (payload.senderName || 'Michael Parker'),
    ...
    console.log('Formatted message to store:', formattedMessage);
    ...
    setVictimMessages(prevMessages => {
      ...
      if (caseMessages.some(msg => msg.id === formattedMessage.id)) {
        console.log('Message already exists in state, skipping:', formattedMessage.id);
        return prevMessages; // Return unchanged state
      }

      const updatedMessages = {
        ...prevMessages,
        [caseId]: [...caseMessages, formattedMessage]
      };
    ...
    if (showChatView && selectedCaseId === caseId) {
      ...
      websocketService.sendMessage('MESSAGE_READ', {
        messageIds: [formattedMessage.id],
  ...
  const [appointments, setAppointments] = useState<Appointment[]>([
    {
      ...
      date: '15/04/2025',
      time: '14:30',
      relatedTo: 'MICHAEL PARKER',
      relatedCase: 'CRI45678/23',
      relatedCrimeType: 'THEFT',
      color: 'bg-blue-500'
    },
    {
      id: 'apt2',
  ...
  const [cases, setCases] = useState<Case[]>(() => {
    ...
    const defaultCases = [
      ...
      },
      {
        ...
        address: '45 Maple Street, London E1 6AZ, UK',
        crimeType: 'THEFT',
        crimeNumber: 'CRI45678/23',
  ...
  const handleOpenCommunication = (method: string, caseId: string, caseName: string) => {
    ...
    if (method === 'message') {
      ...
      // For demo purposes, show a toast notification for other communication methods
      console.log(`Opening ${method} for ${caseName}`);
      setToastMessage(`Opening ${method} for ${caseName}`);
      setToastType('info');
      setShowToast(true);
    }
  };
  ...
  const handleSaveAppointment = (newAppointment: Omit<Appointment, 'id'>) => {
    ...
    const appointmentNotification = {
      ...
      status: 'pending' // Add status field to track appointment response
    };
    ...
    // Show toast notification about the appointment creation
    setToastMessage(`Appointment created: ${appointment.type} on ${appointment.date} at ${appointment.time}. Added to Appointments tab.`);
  ...
  const handleSendMessageToVictim = (message: string) => {
    ...
    const messageId = `police-${Date.now()}`;
  ...
  const handleSubmitNewCase = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!crimeNumber.trim()) {
      setToastMessage('Please enter a crime number');
      setToastType('error');
      setShowToast(true);
      return;
    }
  ...
  // Function to handle task added successfully
  ...
  const handleSendCaseToVictim = (caseItem: Case) => {
    ...
    setToastMessage(`Case ${caseItem.crimeNumber} sent to victim app successfully`);
    setToastType('success');
    setShowToast(true);
  };
  ...
  const handleSendVCOPUpdate = (caseItem: Case) => {
    if (!vcopUpdateMessage.trim()) {
      setToastMessage('Please enter an update message');
      setToastType('error');
      setShowToast(true);
      return;
    }
    ...
    const vcopUpdate = {
      ...
      recipientId: 'victim-michael',
      sender: 'officer',
      vcopUpdate: true // Flag to identify as VCOP update
    };

    // Send VCOP update using VCOP_UPDATE type
    // Important: Structure the payload correctly for the victim app to process
    websocketService.sendMessage('VCOP_UPDATE', {
      ...vcopUpdate,
      recipientId: 'victim-michael',
      crimeNumber: 'CRI45678/23' // Always use the same crime number for routing
    });
    ...
    setToastMessage(`VCOP update sent successfully to ${caseItem.victimName}`);
    setToastType('success');
    setShowToast(true);
  };
  ...
  const handleSendZoorieUpdate = () => {
    ...
    zoorieRelatedCases.forEach(caseItem => {
      // Create a properly structured VCOP update
      const vcopUpdate = {
        id: Date.now().toString(),
        caseId: caseItem.id,
        crimeNumber: caseItem.crimeNumber,
        victimName: caseItem.victimName,
        message: zoorieUpdateInput,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        date: new Date().toLocaleDateString(),
        read: false,
        senderName: 'DC S. Morgan',
      ...
      // Send with the correct structure
      websocketService.sendMessage('ZOORIE_UPDATE', {
        ...vcopUpdate,
        recipientId: 'victim-michael', // For demo, send to Michael Parker
        crimeNumber: 'CRI45678/23' // Always use the same crime number for routing
      });
    });
    ...
    // Show success toast notification
    setToastMessage('Updates have been sent to all ZOORIE victims');
    setToastType('success');
    setShowToast(true);
  };
  ...
  const handleExportOption = (caseId: string, option: 'all' | 'selected' | 'other') => {
    ...
    switch (option) {
      case 'all':
        setToastMessage(`Exporting all data for case: ${caseItem.crimeNumber}`);
        setToastType('info');
        setShowToast(true);
        break;
      case 'selected':
        setToastMessage(`Exporting selected data for case: ${caseItem.crimeNumber}`);
        setToastType('info');
        setShowToast(true);
        break;
      case 'other':
        setToastMessage(`Exporting to other system for case: ${caseItem.crimeNumber}`);
        setToastType('info');
        setShowToast(true);
        break;
    }
  ...
  if (currentView === 'ai-assistant') {
    return (
      <>
        <AIAssistantPage onBack={handleBackToSplash} />
        <ConnectionStatus />
      </>
    );
  }
            ...
            <div className="bg-gray-100 border-b border-gray-200">
              ...
              <div className="flex items-center justify-between p-3">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <button
                    className="bg-blue-600 text-white text-xs font-medium px-2 py-1 rounded-full flex items-center space-x-0.5 hover:bg-blue-700 transition-colors"
              ...
              <div className="px-4 pt-3 pb-1">
                <div className="bg-gray-200 p-1 rounded-lg flex">
                  <button
                    className={`flex-1 py-2 px-1 flex items-center justify-center rounded-lg transition-all duration-200 ${activeTab === 'cases' ? 'bg-blue-600 shadow-sm text-white' : 'text-gray-600'}`}
                    onClick={() => setActiveTab('cases')}
                  >
                    <span className="font-medium text-sm">Active Cases</span>
                    <span className={`ml-1.5 ${activeTab === 'cases' ? 'bg-blue-500' : 'bg-gray-300'} ${activeTab === 'cases' ? 'text-white' : 'text-blue-600'} rounded-full w-6 h-6 inline-flex items-center justify-center text-xs`}>
                      {cases.length}
                    </span>
                  </button>
                  <button
                    className={`flex-1 py-2 px-1 flex items-center justify-center rounded-lg transition-all duration-200 ${activeTab === 'appointments' ? 'bg-blue-600 shadow-sm text-white' : 'text-gray-600'}`}
                    onClick={() => setActiveTab('appointments')}
                  >
                    <span className="font-medium text-sm">Appointments</span>
                    <span className={`ml-1.5 ${activeTab === 'appointments' ? 'bg-blue-500' : 'bg-gray-300'} ${activeTab === 'appointments' ? 'text-white' : 'text-blue-600'} rounded-full w-6 h-6 inline-flex items-center justify-center text-xs`}>
                      {appointments.length}
                    </span>
                  </button>
                </div>
